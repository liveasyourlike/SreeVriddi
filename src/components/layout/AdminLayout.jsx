...
import { 
  LayoutDashboard, Users, UserCheck, Coins, Calculator, FileCheck2, PackageCheck, 
  FileText, Banknote, ShieldAlert, Scale, FolderGit2, Image, Newspaper, BarChart3, 
  History, Settings, ArrowLeft, LogOut, Shield 
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const adminMenu = [
    ...
    { name: 'Customers 360', path: '/admin/customers', icon: UserCheck },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-slate-950 border-r border-amber-500/20 p-4 flex flex-col justify-between flex-shrink-0">
          ...
          <div className="pb-4 mb-4 border-b border-slate-800 flex items-center justify-between">
            <Link to="/" className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="block text-[9px] text-emerald-400 font-bold tracking-widest uppercase">ADMIN & CRM</span>
              </div>
            </Link>
          </div>
        ...
        <div className="pt-4 border-t border-slate-800 space-y-2">
          <Link
            to="/"
            className="flex items-center gap-2 text-xs text-slate-400 hover:text-amber-400 px-3 py-1.5 transition-colors"
          >
          ...
          <Link
            to="/login"
            className="flex items-center gap-2 text-xs text-rose-400 hover:text-rose-300 px-3 py-1.5 transition-colors"
          >
