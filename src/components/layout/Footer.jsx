      ...
      <div className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">A focused footer for brand identity, direct contact and customer policy access — not another navigation menu.</p>
          </div>
          <div className="relative rounded-[2rem] border border-amber-300/20 bg-gradient-to-br from-amber-300/[0.10] via-slate-950/90 to-slate-900/70 p-5 shadow-[0_25px_70px_rgba(0,0,0,.35)] [transform-style:preserve-3d] hover:[transform:perspective(900px)_rotateX(3deg)_rotateY(-3deg)] transition-transform duration-500">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-[.22em] text-amber-300">CONNECT WITH SREE VRIDDHI</p>
                <div className="mt-2 grid gap-1.5 text-xs text-slate-400">
                  <a href={`tel:${brandSettings.phone}`} className="inline-flex items-center gap-2 hover:text-amber-300"><Phone size={13} className="text-amber-300" />{brandSettings.phone}</a>
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2">
          <Link to="/terms" className="group relative min-h-[126px] overflow-hidden rounded-[1.75rem] border border-amber-300/20 bg-gradient-to-br from-amber-300/[0.09] via-slate-950 to-slate-900 p-5 shadow-[0_20px_60px_rgba(0,0,0,.28)] [transform-style:preserve-3d] transition duration-500 hover:-translate-y-1 hover:border-amber-300/50 hover:shadow-[0_25px_70px_rgba(245,158,11,.10)]">
            ...
            <div className="relative flex h-full items-start gap-4">
              ...
              <div><span className="text-[9px] font-black uppercase tracking-[.2em] text-amber-400">01 / CUSTOMER AWARENESS</span><h3 className="mt-2 text-base font-extrabold text-white group-hover:text-amber-200">Customer Risk & Policy Acknowledgement</h3><p className="mt-1 text-[11px] leading-5 text-slate-500">Read the business terms and acknowledge before submitting.</p></div>
            </div>
          </Link>
          <Link to="/privacy" className="group relative min-h-[126px] overflow-hidden rounded-[1.75rem] border border-cyan-300/15 bg-gradient-to-br from-cyan-300/[0.06] via-slate-950 to-slate-900 p-5 shadow-[0_20px_60px_rgba(0,0,0,.28)] [transform-style:preserve-3d] transition duration-500 hover:-translate-y-1 hover:border-cyan-300/40">
            ...
            <div className="relative flex h-full items-start gap-4">
              ...
              <div><span className="text-[9px] font-black uppercase tracking-[.2em] text-cyan-300">02 / PRIVACY & DATA</span><h3 className="mt-2 text-base font-extrabold text-white group-hover:text-cyan-200">Privacy Policy</h3><p className="mt-1 text-[11px] leading-5 text-slate-500">Understand what information is collected, used and protected.</p></div>
            </div>
          </Link>
        </div>

        <div className="mt-7 flex flex-col gap-3 border-t border-white/5 pt-5 text-[10px] text-slate-600 sm:flex-row sm:items-center sm:justify-between">
          <span>© {new Date().getFullYear()} Sree Vriddhi Value Management</span>
          <span className="inline-flex items-center gap-3"><a href={brandSettings.whatsappUrl} target="_blank" rel="noopener noreferrer" className="hover:text-emerald-300"><MessageSquare size={13} className="inline mr-1" />WhatsApp</a><a href={`mailto:${SUPPORT_EMAIL}`} className="hover:text-amber-300">Email</a></span>
        </div>
      </div>
    </footer>
  );
