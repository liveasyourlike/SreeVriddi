...
import { ArrowRight, BarChart3, CheckCircle2, Coins, FileCheck2, Landmark, ShieldCheck, Sparkles, X } from 'lucide-react';
          ...
          {topics.map((topic, index) => {
            ...
            return (
              <div key={topic.title} className="[perspective:1000px] min-h-[150px] sm:min-h-[165px]">
                <div
                  className={`relative h-full min-h-[150px] sm:min-h-[165px] transition-transform duration-500 [transform-style:preserve-3d] ${isActive ? '[transform:rotateY(180deg)]' : ''}`}
                  ...
                  aria-label={`${topic.title}. ${isActive ? 'Close details' : 'Flip to explore'}`}
                >
                  <div className="absolute inset-0 [backface-visibility:hidden] rounded-2xl border border-slate-800 bg-slate-900/85 p-4 sm:p-5 transition-all hover:-translate-y-1 hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-amber-500/25 bg-amber-500/10 text-amber-300">
                        <Icon className="h-4 w-4" />
                      </div>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">{String(index + 1).padStart(2, '0')}</span>
                    </div>
                  ...
                  <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl border border-amber-500/30 bg-slate-950 p-4 sm:p-5 shadow-xl shadow-black/20">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-extrabold uppercase tracking-widest text-amber-400">Explore</span>
                      <button
                        type="button"
                        className="rounded-full p-1 text-slate-400 hover:bg-slate-800 hover:text-white"
                        onClick={(event) => { event.stopPropagation(); setActive(null); }}
                    ...
                    <Link
                      ...
                      className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[10px] font-extrabold uppercase tracking-wider text-amber-300 hover:bg-amber-500/20"
                    >
                      {topic.label}
