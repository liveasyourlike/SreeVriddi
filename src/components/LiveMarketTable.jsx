...
const money = (value, currency = 'INR') => {
  ...
  return new Intl.NumberFormat('en-IN', { style: 'currency', currency, maximumFractionDigits: currency === 'INR' ? 0 : 2 }).format(Number(value));
};
  ...
  return (
    <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 text-slate-300">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
        <div><h4 className="text-sm font-bold text-amber-300">Live Market Rates</h4><p className="text-[10px] text-slate-500">International spot benchmark — not a Hyderabad/jeweller retail quote.</p></div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] text-sm"><thead><tr className="border-b border-slate-800 text-[10px] uppercase tracking-wider text-slate-500"><th className="py-2 text-left font-semibold">Market</th><th className="py-2 text-right font-semibold">Live value</th><th className="py-2 text-right font-semibold">Unit</th></tr></thead>
          <tbody>
            ...
            <tr className="border-b border-slate-800/80"><td className="py-3 text-slate-300">Gold spot</td><td className="py-3 text-right font-bold text-white">{money(data.goldSpotUsdPerOz, 'USD')}</td><td className="py-3 text-right text-xs text-slate-500">per troy oz (XAU/USD)</td></tr>
            <tr className="border-b border-slate-800/80"><td className="py-3 text-slate-300">Gold equivalent</td><td className="py-3 text-right font-bold text-amber-300">{money(data.goldSpotInrPer10g, 'INR')}</td><td className="py-3 text-right text-xs text-slate-500">per 10g, converted spot</td></tr>
      ...
      <div className="mt-3 flex items-center justify-between gap-3"><p className="text-[10px] leading-4 text-slate-500">Gold equivalent = XAU/USD ÷ 31.1034768 × 10 × USD/INR. Retail 22K/24K, MCX and jeweller rates may differ.</p><button onClick={fetchData} disabled={loading} className="shrink-0 rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-300 hover:border-amber-400 disabled:opacity-50">{loading ? 'Refreshing…' : 'Refresh'}</button></div>
    </div>
  );
}
