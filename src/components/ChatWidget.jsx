...
import { Bot, CalendarClock, Check, ChevronLeft, ChevronRight, Clock3, Download, GripVertical, Mail, Maximize2, MessageSquarePlus, Mic, Minimize2, Phone, Send, Sparkles, Trash2, X } from 'lucide-react';
...
const getMonthCells = monthDate => {
  ...
  for (let day = 1; day <= total; day += 1) cells.push(new Date(monthDate.getFullYear(), monthDate.getMonth(), day));
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    const move = e => {
      if (dragRef.current) {
        ...
        setPosition({
          x: Math.max(8, Math.min(window.innerWidth - r.width - 8, e.clientX - r.offsetX)),
          y: Math.max(8, Math.min(window.innerHeight - r.height - 8, e.clientY - r.offsetY))
        });
      }
      if (resizeRef.current) {
        const r = resizeRef.current;
        setSize({
          width: Math.max(360, Math.min(window.innerWidth - 16, e.clientX - r.left)),
          height: Math.max(500, Math.min(window.innerHeight - 16, e.clientY - r.top))
        });
      }
    };
  ...
  const beginResize = e => {
    ...
    resizeRef.current = { left: box.left, top: box.top };
    e.preventDefault();
    e.stopPropagation();
  };
  ...
  const endConversation = () => {
    ...
    updateActive(c => ({ ...c, ended: true }));
    setError('Conversation ended. Select Start New to continue with a new conversation.');
  };
  ...
  const download = () => {
    ...
    a.href = url;
    a.download = `${active.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'sree-vriddhi-chat'}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const chooseDate = d => {
    if (!d || dateKey(d) < todayKey) return;
    updateProfile('callDate', dateKey(d));
    setError('');
  };
  ...
  const requestCall = () => {
    ...
    updateActive(c => ({ ...c, messages: [...c.messages, { role: 'assistant', content: `Call request saved for ${formatDate(profile.callDate)} at ${formatTime(profile.callTime)}. This is a Preview request; staff/calendar confirmation is not connected yet.`, category: 'BUSINESS', at: new Date().toISOString() }] }));
  };
  ...
  const voice = () => {
    ...
    r.onerror = () => { setListening(false); setError('Voice input could not be started.'); };
    ...
    <div className={`relative flex ${collapsed ? 'h-auto' : 'h-full'} w-full flex-col overflow-hidden rounded-2xl border-2 ${dragging ? 'border-amber-400 shadow-[0_0_30px_rgba(245,158,11,.25)]' : 'border-slate-700'} bg-slate-900 shadow-2xl`}>
      ...
      {!collapsed && <div className="flex min-h-0 flex-1 flex-col">
        {!profileReady ? <div className="flex-1 overflow-y-auto p-4"><div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4"><div className="flex items-center gap-2 text-white"><Sparkles className="h-5 w-5 text-amber-400" /><strong>Before we start</strong></div><p className="mt-1 text-xs leading-5 text-slate-400">A few details help the assistant understand what you need.</p></div><form onSubmit={saveProfile} className="space-y-3"><label className="block text-xs text-slate-300">1. Full name<input required value={profile.fullName} onChange={e => updateProfile('fullName', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="Your full name" /></label><label className="block text-xs text-slate-300">2. Email<input required type="email" value={profile.email} onChange={e => updateProfile('email', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="you@example.com" /></label><label className="block text-xs text-slate-300">3. Asset / area of interest<select value={profile.asset} onChange={e => updateProfile('asset', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white"><option value="">Select an option</option>{ASSETS.map(a => <option key={a}>{a}</option>)}</select></label><label className="block text-xs text-slate-300">4. Money you are considering<input value={profile.amount} onChange={e => updateProfile('amount', e.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="Amount (₹)" /></label><button className="w-full rounded-lg bg-amber-500 px-3 py-2.5 text-xs font-semibold text-slate-950">Start conversation</button></form>{error && <div className="mt-3 text-[10px] text-red-300">{error}</div>}</div> : <div className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="shrink-0 border-t border-slate-800 bg-slate-950 p-3"><button type="button" onClick={() => setScheduleOpen(v => !v)} data-no-drag className="flex w-full items-center justify-between rounded-lg border border-slate-800 bg-slate-900/70 px-3 py-2 text-left"><span className="flex items-center gap-2 text-[10px] font-semibold text-slate-200"><CalendarClock className="h-4 w-4 text-amber-400" />5. Schedule a call</span><span className="text-[9px] text-amber-300">{profile.callDate && profile.callTime ? `${formatDate(profile.callDate)} · ${formatTime(profile.callTime)}` : 'Choose date & time'}</span></button>{scheduleOpen && <div className="mt-3 rounded-xl border border-amber-500/20 bg-slate-900 p-3"><div className="mb-2 flex items-center justify-between"><button type="button" onClick={() => setCalendarMonth(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} disabled={!canPrevious} data-no-drag className="rounded-full border border-slate-700 p-1.5 text-slate-300 disabled:opacity-30"><ChevronLeft className="h-4 w-4" /></button><strong className="text-xs text-white">{monthLabel}</strong><button type="button" onClick={() => setCalendarMonth(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))} data-no-drag className="rounded-full border border-slate-700 p-1.5 text-slate-300"><ChevronRight className="h-4 w-4" /></button></div><div className="mb-1 grid grid-cols-7 gap-1 text-center text-[8px] text-slate-500">{['S','M','T','W','T','F','S'].map((d, i) => <span key={`${d}-${i}`}>{d}</span>)}</div><div className="grid grid-cols-7 gap-1">{cells.map((d, i) => { const key = d ? dateKey(d) : `empty-${i}`; const selected = d && dateKey(d) === profile.callDate; const disabled = !d || dateKey(d) < todayKey; return <button key={key} type="button" disabled={disabled} onClick={() => chooseDate(d)} data-no-drag className={`aspect-square rounded-full text-[10px] transition ${selected ? 'bg-amber-500 font-bold text-slate-950 ring-2 ring-amber-300' : 'bg-slate-950 text-slate-200 hover:bg-amber-500/20'} ${disabled ? 'cursor-not-allowed opacity-20' : ''}`}>{d?.getDate()}</button>; })}</div><div className="mt-3 flex items-center gap-2 text-[9px] font-semibold text-slate-300"><Clock3 className="h-3.5 w-3.5 text-amber-400" />Available times <span className="font-normal text-slate-500">(IST)</span></div><div className="mt-2 grid max-h-28 grid-cols-4 gap-1.5 overflow-y-auto pr-1">{TIME_SLOTS.map(t => <button key={t} type="button" disabled={!profile.callDate} onClick={() => { updateProfile('callTime', t); setError(''); }} data-no-drag className={`rounded-full border px-1.5 py-1.5 text-[9px] ${profile.callTime === t ? 'border-amber-400 bg-amber-500 font-semibold text-slate-950' : 'border-slate-700 bg-slate-950 text-slate-300 hover:border-amber-500/50'} disabled:cursor-not-allowed disabled:opacity-30`}>{formatTime(t)}</button>)}</div><div className="mt-3 rounded-lg border border-slate-800 bg-slate-950 p-2 text-center text-[9px] text-slate-400">{profile.callDate && profile.callTime ? <><span className="text-slate-500">Selected:</span> <strong className="text-white">{formatDate(profile.callDate)} · {formatTime(profile.callTime)}</strong></> : <span>Select a date, then a time slot.</span>}</div><button type="button" onClick={requestCall} disabled={!profile.callDate || !profile.callTime} data-no-drag className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg bg-amber-500 px-3 py-2 text-[10px] font-semibold text-slate-950 disabled:opacity-40"><Check className="h-3.5 w-3.5" />Request call at selected time</button><p className="mt-2 text-[8px] leading-4 text-slate-600">Preview mode: the selected slot is saved on this device. Staff/calendar confirmation will be connected in the backend step.</p></div>}</div>
      ...
      <div onPointerDown={beginResize} data-no-drag title="Drag to resize" className="absolute bottom-0 right-0 z-20 flex h-8 w-8 cursor-nwse-resize items-end justify-end rounded-tl-lg border-l-2 border-t-2 border-amber-500/60 bg-slate-950 p-1.5 shadow-lg"><GripVertical className="h-4 w-4 rotate-[-45deg] text-amber-400" /></div>
    </div>
  </div>;
}
