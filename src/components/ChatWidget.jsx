...
export default function ChatWidget() {
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      setWidth(Math.max(300, Math.min(720, dragStart.current.w + dx)));
      setHeight(Math.max(300, Math.min(760, dragStart.current.h + dy)));
    };
  ...
  const sendMessage = async (forcedText) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/chat`, {
        ...
        body: JSON.stringify({
          messages: nextMessages.slice(-12).map(m => ({ role: m.from === 'user' ? 'user' : 'assistant', content: m.text })),
  ...
  return (
    <div style={{ position: 'fixed', right: 20, bottom: 20, zIndex: 60 }}>
      {!open && <button onClick={() => setOpen(true)} aria-label="Open Sree Vriddhi AI chat" className="flex items-center gap-2 px-4 py-3 rounded-full bg-emerald-600 text-white shadow-lg hover:bg-emerald-500"><Bot className="h-4 w-4" /> Chat with us</button>}
      {open && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden" style={{ width, height }}>
          <div className="flex items-center justify-between p-3 bg-slate-950 border-b border-slate-800 cursor-grab" onMouseDown={startDrag}>
            <div className="flex items-center gap-2"><div className="rounded-full bg-emerald-500/15 p-2"><Bot className="h-4 w-4 text-emerald-400" /></div><div><strong className="text-sm text-white">Sree Vriddhi AI Agent</strong><div className="flex items-center gap-1 text-[10px] text-emerald-400"><CheckCircle2 className="h-3 w-3" /> {status}</div></div></div>
          ...
          <div className="p-3 overflow-auto" style={{ height: height - 205 }}>
            ...
            {messages.map((m) => <div key={m.id} className={`mb-3 ${m.from === 'user' ? 'text-right' : 'text-left'}`}><div className={`inline-block max-w-[90%] px-3 py-2 rounded-lg text-xs leading-5 ${m.from === 'user' ? 'bg-amber-500/20 text-amber-200' : 'bg-slate-800 text-slate-200'}`}>{m.text}</div></div>)}
            {sending && <div className="text-[11px] text-amber-300">AI agent is preparing a response…</div>}
          </div>

          <div className="p-3 border-t border-slate-800 bg-slate-950 space-y-2">
            <div className="flex gap-2">
              <button onClick={readPage} title="Read current page aloud" aria-label="Read current page aloud" className="p-2 rounded-lg border border-slate-800 text-slate-300 hover:text-amber-300"><Volume2 className="h-4 w-4" /></button>
              <button onClick={() => document.querySelector('main')?.focus()} title="Focus page content" aria-label="Focus page content" className="p-2 rounded-lg border border-slate-800 text-slate-300 hover:text-amber-300"><Eye className="h-4 w-4" /></button>
