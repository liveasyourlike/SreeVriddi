  ...
  return <div className="fixed right-3 bottom-3 z-[60] max-w-[calc(100vw-24px)]">
    ...
    {open && <div className="w-[min(420px,calc(100vw-24px))] overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-2xl">
      ...
      <form onSubmit={send} className="border-t border-slate-800 bg-slate-950 p-3">
        ...
        <div className="mt-2 flex items-center justify-between text-[9px] text-slate-600"><span>Public information only · human review when needed</span><span>{message.length}/4000</span></div>
        <div className="mt-2 grid grid-cols-2 gap-2"><Link to="/eligibility" onClick={() => setOpen(false)} className="flex items-center justify-center gap-1 rounded-lg border border-amber-500/20 bg-amber-500/5 px-2 py-2 text-[10px] font-semibold text-amber-300"><ClipboardCheck className="h-3 w-3" /> Evaluate</Link><a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-2 py-2 text-[10px] font-semibold text-emerald-300"><Phone className="h-3 w-3" /> Human support</a></div>
      </form>
    </div>}
  </div>;
}
