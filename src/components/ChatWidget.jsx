...
import { Bot, CalendarDays, ChevronLeft, ChevronRight, Download, ExternalLink, FileUp, Mic, Minus, Phone, Plus, Send, UserRound, X } from 'lucide-react';
...
function Sources({ sources = [] }) {
  ...
  return (
    <div className="sv-sources">
      <span className="sv-sources-label">Sources &amp; actions</span>
      <div className="sv-source-list">
        {sources.map((s, i) => <a key={`${s.url}-${i}`} href={s.url} target="_blank" rel="noopener noreferrer" className="sv-source-link"><ExternalLink />{s.label}</a>)}
      </div>
    </div>
  );
}
...
export default function ChatWidget() {
  ...
  const persist = (next = {}) => writeStore({
    ...readStore(), profile: next.profile ?? profile, messages: next.messages ?? messages,
    attachments: next.attachments ?? attachments, sessions: next.sessions ?? sessions,
  });
  ...
  const start = () => {
    ...
    const p = existing || { id: crypto.randomUUID?.() || String(Date.now()), name: draft.name, email: draft.email, asset: draft.asset, amount: draft.amount, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    const contacts = [...(s.contacts || []).filter((c) => c.email.toLowerCase() !== draft.email.toLowerCase()), { ...p, name: draft.name || p.name, asset: draft.asset || p.asset, amount: draft.amount || p.amount, updatedAt: new Date().toISOString() }];
    ...
    setProfile(p); setMessages(next); setSessions(ns); setActive(ns.filter((x) => x.status === 'active').length);
  };
  ...
  const send = async () => {
    ...
    setMessages(next); setInput(''); setError(''); setLoading(true); persist({ messages: next });
    try {
      ...
      const all = [...next, { role: 'assistant', content: data.answer, risk: data.risk, requiresHuman: data.requiresHuman, sources: data.sources || [] }];
      setMessages(all); persist({ messages: all });
    } catch (e) { setError(e.message || 'AI service unavailable'); }
  ...
  const upload = (e) => {
    ...
    if (bad) { setFileError(`${bad.name} exceeds the 10 MB per-file limit.`); return; }
    setFileError('');
    const out = files.map((f) => ({ id: crypto.randomUUID?.() || String(Date.now() + Math.random()), name: f.name, size: f.size, type: f.type || 'application/octet-stream', email: profile?.email || '', data: null, uploadedAt: new Date().toISOString() }));
    const next = [...attachments, ...out]; setAttachments(next); persist({ attachments: next }); e.target.value = '';
  };

  const downloadConversation = () => downloadBlob(`sree-vriddhi-conversation-${Date.now()}.txt`, messages.map((m) => `${m.role.toUpperCase()}: ${m.content}`).join('\n\n'));

  const schedule = () => {
    if (!date || !time || !profile) return;
    const bookingItem = { id: crypto.randomUUID?.() || String(Date.now()), date, time, email: profile.email, name: profile.name, createdAt: new Date().toISOString(), status: 'requested' };
    const store = readStore(); writeStore({ ...store, bookings: [...(store.bookings || []), bookingItem] });
    setBooking(false);
    setMessages((current) => [...current, { role: 'assistant', content: `Call request saved for ${date} at ${time} (IST). Sree Vriddhi will confirm the appointment. Please note this is a request, not a confirmed calendar booking.` }]);
    setDate(''); setTime('');
  };

  const end = () => {
    const ns = sessions.map((x) => x.email === profile?.email && x.status === 'active' ? { ...x, status: 'ended', endedAt: new Date().toISOString() } : x);
    setSessions(ns); setActive(ns.filter((x) => x.status === 'active').length); persist({ sessions: ns });
  };

  const newChat = () => {
    setProfile(null); setDraft({ name: '', email: '', asset: '', amount: '' }); setMessages([]); setAttachments([]); setInput('');
    setBooking(false); setDate(''); setTime(''); setError(''); setFileError('');
  };

  const voice = () => {
    const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!Recognition) { setError('Voice input is not supported by this browser.'); return; }
    const recognition = new Recognition(); recognition.lang = 'en-IN'; recognition.onstart = () => setListening(true); recognition.onend = () => setListening(false); recognition.onerror = () => setError('Voice input could not be started.'); recognition.onresult = (e) => setInput(e.results?.[0]?.[0]?.transcript || ''); recognition.start();
  };

  return (
    <div className="sv-chat-wrap">
      {!open && <button className="sv-launch" onClick={() => setOpen(true)}><Bot /> AI Assistant</button>}
      {open && <div className={`sv-chat ${min ? 'collapsed' : ''}`}>
        <header className="sv-header">
          <div className="sv-brand"><div className="sv-logo"><img src="/brand/logo-mark.jpeg" alt="Sree Vriddhi" /></div><div><b>Sree Vriddhi AI</b><small>AI Assistant · {active} active</small></div></div>
          <div className="sv-actions"><button title="Download conversation" onClick={downloadConversation}><Download /></button><button title="Start new conversation" onClick={newChat}><Plus /></button><button title="Collapse" onClick={() => setMin(!min)}><Minus /></button><button title="Close" onClick={() => setOpen(false)}><X /></button></div>
        </header>
        ...
        {!min && <>
          {!profile ? <section className="sv-prechat">
            ...
            <main ref={scrollRef} className="sv-messages">
              {messages.map((m, i) => <div key={i} className={`sv-row ${m.role}`}>
                <div className="sv-meta">{m.role === 'assistant' ? <><Bot /> Sree Vriddhi AI</> : <><UserRound /> You</>}</div>
                <div className="sv-bubble"><MessageContent content={m.content} /></div>
            ...
            <section className="sv-booking">
              ...
              {booking && <div className="sv-booking-body"><CalendarPicker value={date} onChange={(selectedDate) => { setDate(selectedDate); setTime(''); }} /><div className="sv-times"><b>{date ? 'Choose a time' : 'Select a date first'}</b><div>{date && TIMES.map((t) => <button type="button" className={time === t ? 'selected' : ''} key={t} onClick={() => setTime(t)}>{t}</button>)}</div></div><div className="sv-selected">{date && time ? `${date} · ${time} IST` : 'Choose a date and time'}</div><button className="sv-primary" disabled={!date || !time} onClick={schedule}>Request call</button></div>}
            </section>

            <footer className="sv-composer">
              <div className="sv-input"><button title="Attach file" onClick={() => fileRef.current?.click()}><FileUp /></button><input type="file" ref={fileRef} hidden multiple onChange={upload} /><textarea rows={1} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={key} placeholder="Type your message…" /><button title="Voice input" onClick={voice}><Mic className={listening ? 'on' : ''} /></button><button className="send" disabled={!input.trim() || loading} onClick={send}><Send /></button></div>
