...
function Protected({ children }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/crm/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token }) })
      .then(async response => { const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'Invalid CRM session'); return data })
      .then(data => { localStorage.setItem('sv_crm_role', data.role); setSession(data); setState('authenticated') })
      .catch(() => { localStorage.removeItem('sv_crm_token'); localStorage.removeItem('sv_crm_role'); setState('unauthenticated') })
  }, [])
...
export default function CRMWorkspace() {
  ...
  const stats = [ ['Contacts', contacts.length, Users], ['Conversations', sessions.length, MessageSquare], ['Call requests', bookings.length, CalendarDays], ['Active chats', sessions.filter(s => s.status === 'active').length, ShieldCheck] ]
